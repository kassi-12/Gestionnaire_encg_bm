...
// Handle the deletion of a rattrapage
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["delete"])) {
    $rattrapage_id = $_POST["rattrapage_id"];
    $delete_sql = "DELETE FROM rattrapage WHERE id = $rattrapage_id";
    if ($conn->query($delete_sql) === TRUE) {
        echo "Rattrapage supprimé avec succès";
    } else {
        echo "Erreur lors de la suppression du rattrapage: " . $conn->error;
    }
}
...
if (!empty($salle_filter) && $salle_filter != "Toutes les salles") {
    $sql = "SELECT r.id, s.name AS salle_name, r.start_time, r.end_time, p.first_name, p.last_name, r.rattrapage_date, g.name AS group_name, g.year, g.filiere, g.extra_info
            FROM rattrapage r
            JOIN salles s ON r.salle_id = s.id
            JOIN reservation res ON r.reservation_id = res.id
            JOIN professeur p ON res.professeur_id = p.id
            JOIN grp g ON res.group_id = g.id
            WHERE r.rattrapage_date = '$date_filter' AND s.name = '$salle_filter'";
} else {
    $sql = "SELECT r.id, s.name AS salle_name, r.start_time, r.end_time, p.first_name, p.last_name, r.rattrapage_date, g.name AS group_name, g.year, g.filiere, g.extra_info
            FROM rattrapage r
            JOIN salles s ON r.salle_id = s.id
            ...
            JOIN professeur p ON res.professeur_id = p.id
            JOIN grp g ON res.group_id = g.id
            WHERE r.rattrapage_date = '$date_filter'";
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["enregistrer"])) {
    class PDF extends FPDF {
        ...
        // Simple table
        // Updated BasicTable function to include filiere and extra_info
        function BasicTable($header, $data) {
            // Set column widths
            $column_widths = [30, 20, 25, 25, 40, 50];  // Adjusted column widths for better fit
            ...
            // Data
            foreach ($data as $row) {
                $this->Cell($column_widths[0], 6, $row['rattrapage_date'], 1, 0, 'C');
    ...
    $header_title = "Salle " . $salle_filter    . " - Rattrapage";
...
.delete-btn {
    background-color: #ff3333; /* Red for delete */
}
                        ...
                        while ($row = $result->fetch_assoc()) {
                            ...
                            echo "<td>
                                    <form method='POST' action='' style='display:inline;'>
                                        ...
                                        <input type='hidden' name='rattrapage_id' value='" . $row["id"] . "'>

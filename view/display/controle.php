...
if (!empty($salle_filter) && $salle_filter != "Toutes les salles") {
    $sql = "SELECT c.id, s.name AS salle_name, c.start_time, c.end_time, p.first_name, p.last_name, c.controle_date, g.name AS group_name, g.year, g.filiere, g.extra_info
            FROM controle c
            JOIN reservation r ON c.reservation_id = r.id
            JOIN professeur p ON r.professeur_id = p.id
            JOIN grp g ON r.group_id = g.id
            JOIN salles_controle sc ON sc.controle_id = c.id
            JOIN salles s ON sc.salle_id = s.id
            WHERE c.controle_date = '$date_filter' AND s.name = '$salle_filter'";
} else {
    $sql = "SELECT c.id, s.name AS salle_name, c.start_time, c.end_time, p.first_name, p.last_name, c.controle_date, g.name AS group_name, g.year, g.filiere, g.extra_info
            FROM controle c
            JOIN reservation r ON c.reservation_id = r.id
            ...
            JOIN salles_controle sc ON sc.controle_id = c.id
            JOIN salles s ON sc.salle_id = s.id
            WHERE c.controle_date = '$date_filter'";
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["enregistrer"])) {
    class PDF extends FPDF {
        ...
        function BasicTable($header, $data) {
            // Set column widths
            $column_widths = [30, 20, 25, 25, 50, 30];  // Adjusted column widths for better fit
            ...
            // Data
            foreach ($data as $row) {
                $this->Cell($column_widths[0], 6, $row['controle_date'], 1, 0, 'C');
    ...
    $header_title = "Salle " . $salle_filter . " - Controles";
